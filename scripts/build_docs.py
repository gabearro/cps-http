...
def search_paths() -> list[Path]:
    ...
    for project in ROOT.parent.glob("cps-*"):
        if (project / "src").is_dir(): result.add(project / "src")
    packages = Path.home() / ".nimble" / "pkgs2"
    if packages.is_dir():
        for package in packages.iterdir():
            if package.is_dir():
                result.add(package)
                if (package / "src").is_dir(): result.add(package / "src")
...
def main() -> None:
    ...
    if not nim: raise SystemExit("nim is required to generate the API reference")
    ...
    with tempfile.TemporaryDirectory(prefix="cps-nimdoc-") as temporary:
        ...
        for number, module in enumerate(modules):
            subprocess.run([str(nim), "doc", "--index:on", "--docCmd:skip", f"--docRoot:{ROOT / 'src'}", f"--lib:{patched_library}", f"--outdir:{output}", f"--nimcache:{temporary_root / f'cache-{number}'}", *paths, str(module)], cwd=ROOT, check=True)
